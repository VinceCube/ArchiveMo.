    ...
    ?>
      ...
      <main id="main">
        ...
        function getExistingFile($email)
        {
          ...
          $query = "SELECT medical_cert FROM medical WHERE email = ?";
        ...
        if (isset($_POST['submit'])) {
          if (!$existing_file) {
            ...
            $sql = "INSERT INTO `medical` (`id`, `email`, `medical_cert`, `date`) VALUES (NULL, ?, ?, ?)";
            $stmt = mysqli_prepare($conn, $sql);
            mysqli_stmt_bind_param($stmt, "sss", $email, $pname, $date);

            if (mysqli_stmt_execute($stmt)) {
              $_SESSION['message'] = '<script>
                swal("Success!", "Health Examination Card successfully submitted.", "success");
            </script>';
              echo "<script>window.location.href = 'medical_form.php';</script>";
              exit();
            } else {
        ...
        <section id="narrative" class="narrative">
          <div class="container">
            ...
            if (isset($_GET['id'])) {
              ...
              $sql = "SELECT medical_cert FROM medical WHERE id = ?";
                ...
                <div class="card recent-sales overflow-auto">
                  <div class="card-body">
                    <table class="table datatable">
                      ...
                      <tbody>
                        <?php
                        $result = mysqli_query($conn, "SELECT * FROM medical WHERE medical.email = '$email'");
                        while ($row = mysqli_fetch_array($result)) {
                        ?>
                          <tr>
                            <td scope="row"><?php echo $row["id"]; ?></td>
                            <td scope="row"><?php echo $row["email"]; ?></td>
                            <td><a href="?id=<?php echo $row['id']; ?>"><?php echo $row['medical_cert'];?></td>
                            <td scope="row"><?php echo $row["date"]; ?></td>
                            <td><a href="javascript:void(0);" class="btn btn-primary viewmodal" data-id="<?php echo $row['id']; ?>" data-waiver="<?php echo $row['medical_cert']; ?>">VIEW</a></td>
